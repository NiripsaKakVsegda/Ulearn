...
class GroupScores extends Component {
	...
	renderCheckbox() {
		...
		const initialChecked = score[mapToServerName.groupScores];
		const isChecked = initialChecked || score[mapToServerName.allGroupScores] || false;
		const isDisabled = score[mapToServerName.allGroupScores] || !score[mapToServerName.unitScores];

		return (
			<React.Fragment>
				<Checkbox initialIndeterminate={ initialChecked === undefined } checked={ isChecked }
						  disabled={ isDisabled }
